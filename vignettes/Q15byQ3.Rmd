...
g2_data = g_data %>%
  select(question, No, Yes) %>%
  tidyr::gather(Farmer, Priority, -question)

g_survey = ggplot(g2_data, aes(x = question,
                               y = Priority,
                               color = Farmer)) +
  geom_point() +
  theme_bw() +
  ...
  theme(
    ...
    legend.background = element_rect(fill=NA, color=NA),
    # legend.key.width = unit(-1,"line"),
    plot.margin=unit(c(0,0.02,-0.45,0.1), "cm"),
    panel.grid.major.x = element_blank()
    ) +
  guides(color = guide_legend(override.aes = list(size=1))) + 
  labs(x='') + 
  scale_color_manual(values = cp3, drop=FALSE) +
  coord_flip()

g_survey

ggsave("Fig4_alt.tiff",
       dpi = 300, height = 7, width = 8.7, units ="cm")
```
